...
class WasmSoundTouch {
    ...
    private allocInput( frames: number ): void {
        if ( !this.exports || this.inputFrames >= frames ) {
            return;
        }
        if ( this.inputPtr ) {
    ...
    private allocOutput( frames: number ): void {
        if ( !this.exports || this.outputFrames >= frames ) {
            return;
        }
        if ( this.outputPtr ) {
    ...
    private drainWasmToFifo(): void {
        for ( ;; ) {
            const received = this.exports!.soundtouch_receive_samples( this.handle, this.outputPtr, this.outputFrames );
            if ( received === 0 ) {
                break;
            }
...
function paramValue( params: WorkletParams, name: keyof WorkletParams, fallback: number ): number {
    ...
    return arr !== undefined && arr.length > 0 ? arr[0] : fallback;
}
...
class SoundTouchProcessor extends AudioWorkletProcessor {
    ...
    constructor() {
        ...
        this.port.onmessage = ( e: MessageEvent ) => {
            ...
            if ( msg?.type === "load-wasm" && msg.wasm instanceof ArrayBuffer ) {
                const ok = this.wasmPipe.init( msg.wasm, sampleRate );
    ...
    process( inputs: Float32Array[][], outputs: Float32Array[][], parameters: WorkletParams ): boolean {
        ...
        if ( input === undefined || input.length === 0 || input[0] === undefined ) {
            leftOut.fill( 0 );
            if ( rightOut !== leftOut ) {
                rightOut.fill( 0 );
            }

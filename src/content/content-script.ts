...
declare global {
    interface Window { "__pitchShifterAttached"?: boolean }
}
...
class PitchShifterContentScript {
    ...
    private isParamsUpdateMessage( message: unknown ): message is ParamsUpdateMessage {
        ...
        if ( m.params === null || typeof m.params !== "object" ) {
            return false;
        }
        const p = m.params as {
            "pitch"?:     unknown;
    ...
    private async connectElement( element: HTMLMediaElement ): Promise<void> {
        ...
        try {
            ...
            source.connect( workletNode ).connect( effectGain ).
